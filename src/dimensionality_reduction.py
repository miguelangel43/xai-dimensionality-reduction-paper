...
def apply_lle(X_train, X_test, n, k, _reg):
...
def apply_kpca(X_train, X_test, n, type_kernel, gamma=None):
...
def apply_lol(X_train, X_test, y_train, n_components):
    ...
    for n_components in pbar:
        ...
        if 'SLMVP' in models_list:
            ...
            reduced_data[key] = apply_slmvp(
                X_train, X_test, y_train, n_components, 'radial', gammas=0.01, multilabel=multilabel)
            ...
            reduced_data[key] = apply_slmvp(X_train, X_test, y_train,
                                            n_components, 'radial', gammas=0.1, multilabel=multilabel)

        if 'PCA' in models_list:
            key = (str(n_components) + 'Dim', 'PCA', '')
            pbar.set_description(str(key))
            reduced_data[key] = apply_pca(X_train, X_test, n_components)

        if 'KPCA' in models_list:
            key = (str(n_components) + 'Dim', 'KPCA', 'Radial')
            pbar.set_description(str(key))
            reduced_data[key] = apply_kpca(
                X_train, X_test, n_components, 'rbf')
        ...
        if 'LLE' in models_list:
            ...
            reduced_data[key] = apply_lle(
                X_train, X_test, n_components, k, reg)
